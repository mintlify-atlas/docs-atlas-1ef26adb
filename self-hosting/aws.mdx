...
<Steps>
  <Step title="Clone the Trieve repository">
    ```sh
  ...
  <Step title="Provision Terraform">
    AWS should be configured with your IAM credentials chosen. Run the following commands to create the EKS cluster:
  ...
  <Step title="Login to the cluster">
  ...
  <Step title="Install Ingress nginx + Cert Manager">
  ...
  <Step title="Setup StorageClass">
    ```sh
    kubectl patch storageclass gp2 -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"true"}}}'
    ```
  </Step>

  <Step title="Download values.yaml file">
    ```sh
  ...
  <Step title="Modify domain names for ingresses">
  ...
  <Step title="Get Ingress IP address">
    ```sh
    kubectl get ingress
    ```
  ...
  <Step title="Add A records">
    ...
    To configure the redirect urls, see Setup Keycloak redirect urls below.
    </Warning>
  </Step>
</Steps>
